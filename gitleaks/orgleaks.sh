...
if [ "$org" == "" ]; then
  ...
  mkdir -p "$current_dir"/reports/"$org"/"$dateTime"
  ...
  if [ -z "$pages" ]
  then
      curl -H 'Authorization: token '"$access_token" -H 'Accept:application/vnd.github.VERSION.raw' -s https://api.github.com/orgs/$org/repos\?per_page\=500 | grep "full_name" | awk -v org="$org" -F"\"" '{print "https://github.com/"$4}' > "$current_dir"/reports/"$org"/"$dateTime"/$org-repos.txt
  else
      > "$current_dir"/reports/"$org"/"$dateTime"/$org-repos.txt
      for (( counter=1; counter<=$pages; counter++ ))
      do  
        curl -H 'Authorization: token '"$access_token" -H 'Accept:application/vnd.github.VERSION.raw' -s https://api.github.com/orgs/$org/repos\?per_page\=500\&page\=$counter | grep "full_name" | awk -v org="$org" -F"\"" '{print "https://github.com/"$4}' >> "$current_dir"/reports/"$org"/"$dateTime"/$org-repos.txt
      done
  fi
  if [ "$args" == "" ]; then
    if [ -z "$fileName" ]
    then
      ...
      done < "$current_dir"/reports/"$org"/"$dateTime"/$org-repos.txt
    else  
      ...
      do 
        ...
        localPath="$current_dir"/reports/"$org"/"$dateTime"/"$localReport"."$format"
        gitleaks --repo-url="$line" --report="$localPath" --format="$format" --access-token=$access_token 
      done < "$current_dir"/reports/"$org"/"$dateTime"/$org-repos.txt
      cat "$current_dir"/reports/"$org"/"$dateTime"/*."$format" > "$current_dir"/reports/"$org"/"$dateTime"/"$fileName"."$format"
    fi
  else 
    if [ -z "$fileName" ]
    then
